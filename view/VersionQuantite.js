...
function checkInput() {
  // Regex
  let checkString = /^[^@&"()!_$*€£`%+=\/;?#]+$/;
  let checkMail = /^[a-zA-Z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$/;
  let checkAdresse = /^[^@&"()!_$*€£`%+=\/;?#]+$/;
...
// Tester les inputs de l'utilisateur
  ...
  alert('Le champs NOM ne doit pas contenir de caractères interdits tels que :'+'[^@&"()!_$*€£`%+=/;?#]'+"ou n'est pas renseigné !");
  return false;
  } else if (checkString.test(prenom) == false) {
    alert('Ce champs PRENOM ne doit pas contenir de caractères interdits tels que :'+'[^@&"()!_$*€£`%+=/;?#]'+ "ou n'est pas renseigné !" );
    return false;
    ...
    alert(
    `Le champs ADRESSE contient un ou plusieurs des caractères interdits suivants : ` +
      '[^@&"()!_$*€£`%+=/;?#]' +
